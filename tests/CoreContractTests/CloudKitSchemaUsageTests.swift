...
final class CloudKitSchemaUsageTests: XCTestCase {
    func testCloudKitCodeUsesSharedSchemaConstants() throws {
        let paths = [
            ...
            "Sources/macOS/App/DeviceManager.swift",
            "Sources/macOS/App/DeviceManager+CloudKit.swift",
        ]
        ...
        for path in paths {
            ...
            for pattern in forbiddenPatterns {
                XCTAssertNil(
                    ...
                    "\(path) should use GetBoredIdentifiers.CloudKit constants instead of matching raw CloudKit schema literal: \(pattern)"
                )
            }
        }
    }
